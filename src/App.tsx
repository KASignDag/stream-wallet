    ...
    <main className="page-shell">
      <section className="wallet-frame" aria-label="Stream Wallet mainnet app">
        ...
        {walletSheet === "send" && <div className="modal-backdrop"><section className="setup-sheet" role="dialog" aria-modal="true" aria-labelledby="send-title"><SheetClose disabled={sendStep === "preparing" || sendStep === "signing"} onClose={() => { setWalletSheet(null); setError(""); }} />
          {sendStep === "compose" && <><span className="eyebrow">LIVE MAINNET PAYMENT</span><h2 id="send-title">Send ZKAS</h2><p>Available: {formatSompi(spendableSompi)} ZKAS. Type the recipient address or scan its QR code. The network fee is calculated before you approve the payment.</p><label className="field-label" htmlFor="send-address">Recipient address</label><textarea id="send-address" value={sendTo} onChange={(event) => { setSendTo(event.target.value.trim()); setError(""); }} rows={3} autoCapitalize="none" autoCorrect="off" autoComplete="off" spellCheck={false} placeholder="zkas:…" /><button className="secondary-action scan-address-action" type="button" disabled={scanning} onClick={scanRecipient}>{scanning ? <LoaderCircle className="spin" size={18} /> : <ScanLine size={18} />} {scanning ? "Opening camera" : "Scan recipient QR code"}</button><label className="field-label" htmlFor="send-amount">Amount in ZKAS</label><input id="send-amount" className="text-input" inputMode="decimal" value={sendAmount} onChange={(event) => { setSendAmount(event.target.value.replace(/[^0-9.]/g, "")); setError(""); }} placeholder="1.00" />{error && <div className="form-error" role="alert">{error}</div>}<button className="primary-action" type="button" disabled={!sendTo || !sendAmount} onClick={prepareSend}>Calculate fee and review <ChevronRight size={18} /></button></>}
